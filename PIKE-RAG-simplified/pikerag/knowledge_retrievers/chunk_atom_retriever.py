...
class ChunkAtomRetriever(BaseQaRetriever, ChromaMixin):
    ...
    def _atom_info_tuple_to_class(self, atom_retrieval_info: List[Tuple[str, Document, float]]) -> List[AtomRetrievalInfo]:
        # 原代码，当召回内容为空（没有找到相关内容时）会报异常，终端验证，为了保证验证集能正常跑完，修改如下代码，当召回内容为空时，跳过处理。
        print(f'atom_retrieval_info:{atom_retrieval_info}')
        if not atom_retrieval_info or len(atom_retrieval_info)==0:
            print('no atom retrieval info')
            return []
